...
class Injector_SequentialLoad extends Simulation {
  ...
  setUp(
    ...
    postRequest.scnGetStory.inject(rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST)
    ...
    postRequest.scnCreateStory.inject( nothingFor(((steadyTime+rampTime)*3)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    postRequest.scnGetStory.inject( nothingFor(((steadyTime+rampTime)*4)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    postRequest.scnCreateCampaign.inject( nothingFor(((steadyTime+rampTime)*5)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    postRequest.scnGetCampaign.inject( nothingFor(((steadyTime+rampTime)*6)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    postRequest.scnCreateImpactFund.inject( nothingFor(((steadyTime+rampTime)*7)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    postRequest.scnGetImpactFund.inject( nothingFor(((steadyTime+rampTime)*8)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    postRequest.scnGetOrganization.inject( nothingFor(((steadyTime+rampTime)*9)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    postRequest.scnUpdatePrefCauses.inject( nothingFor(((steadyTime+rampTime)*10)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    postRequest.scnHomePageSettings.inject( nothingFor(((steadyTime+rampTime)*11)+sleepTime seconds),rampUsersPerSec(1) to (peakRPS_SequentialLoad_LowRPS) during (rampTime seconds), constantUsersPerSec(peakRPS_SequentialLoad_LowRPS) during(steadyTime seconds)).protocols(httpTEST),
    ...
    postRequest.scnQueryJobHistory.inject( nothingFor(((steadyTime+rampTime)*13)+sleepTime seconds), rampUsersPerSec(1) to (peakRPS_SequentialLoad) during (rampTime seconds),constantUsersPerSec(peakRPS_SequentialLoad) during(steadyTime seconds)).protocols(httpTEST),

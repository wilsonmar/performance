...
class Injector extends Simulation {
  ...
  setUp(
    ...
    details(postRequest.grpCompaign / "CreateCompaign" ).responseTime.mean.lte( meanResponseTime),
    details(postRequest.grpCompaign / "CreateCompaign" ).failedRequests.percent.lte( errorRate),
    details(postRequest.grpCompaign / "GetCompaign" ).responseTime.mean.lte( meanResponseTime),
    details(postRequest.grpCompaign / "GetCompaign" ).failedRequests.percent.lte( errorRate),
